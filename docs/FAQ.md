...
### Зачем нужен jq на роутере Keenetic?

jq - это утилита для корректного парсинга JSON. Она необходима на роутерах Keenetic для:
...
Устанавливается автоматически при развертывании Phobos на роутере Keenetic.
...
jq требуется для корректного парсинга JSON при работе с RCI API роутера Keenetic.
...
jq на OpenWrt не требуется, так как используется UCI для настройки.
